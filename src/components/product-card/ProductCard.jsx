...
const ProductCard = ({product, deleteButton}) => {
  ...
  return (
    <div>
            <section className='product__top'>
              ...
              {  wishlist?.find(({_id}) => _id === product?._id) ? <button onClick={() => removeFromWishlist(product?._id)} className='product__wishlist'> { deleteButton ? <RiDeleteBin7Fill /> : <AiFillHeart />} </button> : <button onClick={() => addToWishlist(product?._id)} className='product__wishlist'> <AiOutlineHeart /> </button>}
